...
binary_search <- function(
  ...
  run_clustering <- function(spe, resolution, ...) {
    num_runs <<- num_runs + 1
    do_clustering(spe, resolution = resolution, ...)
  }
  ...
  if (!is.null(resolution_boundaries)) {
    ...
    res <- resolution_init
    result <- run_clustering(spe, resolution = res, ...)
  ...
  while ((rb - lb > tolerance || lb == rb) && i < num_rs) {
    ...
    result <- run_clustering(spe, resolution = mid, ...)
    n_clust <- extract_nclust(result)
  ...
  return(list(result = result, resolution = mid, num_runs = num_runs))
}
